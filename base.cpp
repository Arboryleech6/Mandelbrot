...
static constexpr int    GAUSS_RADIUS = 63;     // kernel 63x63 — filtro pesado
...
// ════════════════════════════════════════════════════════════════════════════
// TAREA A — Renderizado Mandelbrot
// ════════════════════════════════════════════════════════════════════════════
Image renderMandelbrot() {
    ...
    for (int py = 0; py < IMG_H; ++py) {
        ...
        for (int px = 0; px < IMG_W; ++px) {
            ...
            // Prueba cardioide principal y primer bulbo (p2) — evita iteraciones
            // en las dos regiones densas del interior sin coste numérico.
            {
...
static void chunk(std::vector<uint8_t>& f, const char ty[4],
                  const std::vector<uint8_t>& d) {
    u32be(f, static_cast<uint32_t>(d.size()));
    const auto* tp = reinterpret_cast<const uint8_t*>(ty);
    f.insert(f.end(), tp, tp+4);
    f.insert(f.end(), d.begin(), d.end());
    uint32_t c = crc32(tp, 4);
    c = crc32(d.data(), d.size(), c ^ 0xFFFFFFFFu);  // continue CRC
    // Recompute correctly: crc over type+data
    std::vector<uint8_t> td(4 + d.size());
    memcpy(td.data(), ty, 4);
    memcpy(td.data()+4, d.data(), d.size());
    u32be(f, crc32(td.data(), td.size()));
    // Remove the wrong CRC we pushed — undo last 4 bytes and push correct
    f.resize(f.size() - 4);
    u32be(f, crc32(td.data(), td.size()));
}

// Wrapper limpio sin el bug de arriba
...
// ════════════════════════════════════════════════════════════════════════════
// TAREA B — Filtros de convolución 2D
// ════════════════════════════════════════════════════════════════════════════
